...
console.log(args[0])}
if (!fs.existsSync('./HachiJadiBot/'+ id)){
fs.mkdirSync('./HachiJadiBot/'+ id, { recursive: true })}
args[0] && args[0] != undefined ? fs.writeFileSync('./HachiJadiBot/' + id + '/creds.json', JSON.stringify(JSON.parse(Buffer.from(args[0], 'base64').toString('utf-8')), null, '\t')) : ''

if (fs.existsSync('./HachiJadiBot/' + id + '/creds.json')) {
let creds = JSON.parse(fs.readFileSync("./HachiJadiBot/" + id + "/creds.json"))
if (creds) {
if (creds.registered = false) {
fs.unlinkSync('./HachiJadiBot/' + id + '/creds.json')
}}}
...
args[0] ? fs.writeFileSync('./HachiJadiBot/' + id + '/creds.json', JSON.stringify(JSON.parse(Buffer.from(args[0], 'base64').toString('utf-8')), null, `\t`)) : ''
...
conversation: 'HachikoBotLite-MD',
}}}
...
console.log(reason)
if (reason == 405) {
await fs.unlinkSync('./HachiJadiBot/' + id + '/creds.json')
...
await sleep(5000)
if (!args[0]) parentw.sendMessage(m.chat, {text : usedPrefix + command + ' ' + Buffer.from(fs.readFileSync('./HachiJadiBot/' + id + '/creds.json'), 'utf-8').toString('base64')}, { quoted: fkontak })    
